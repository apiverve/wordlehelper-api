...
{
    class WordleHelperExample
    {
        private static readonly string API_KEY = Environment.GetEnvironmentVariable("APIVERVE_API_KEY") ?? "YOUR_API_KEY_HERE";
        private static readonly string API_URL = "https://api.apiverve.com/v1/wordlehelper";

        /// <summary>
        /// Make a GET request to the Wordle Helper API
        /// </summary>
        static async Task<JsonDocument> CallWordleHelperAPI()
        {
            try
            {
        ...
        {
            Console.WriteLine("📤 Calling Wordle Helper API...\n");

            var result = await CallWordleHelperAPI();
