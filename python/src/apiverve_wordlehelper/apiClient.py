...
class WordlehelperAPIClientError(Exception):
...
class ValidationError(WordlehelperAPIClientError):
...
class WordlehelperAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {
        "green": {"type": "string", "required": False},
        "yellow": {"type": "string", "required": False},
        "gray": {"type": "string", "required": False},
        "pattern": {"type": "string", "required": False, "minLength": 5, "maxLength": 5},
        "limit": {"type": "integer", "required": False, "min": 1, "max": 100, "default": 25}
    }
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the WordlehelperAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises WordlehelperAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/wordlehelper'
        self.headers = {
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise WordlehelperAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[WordlehelperAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns WordleHelperData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises WordlehelperAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise WordlehelperAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise WordlehelperAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise WordlehelperAPIClientError(f"Request failed: {str(e)}")
